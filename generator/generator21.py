...
from schema21 import *

import inspect

from collections import namedtuple

# OCPP 2.1 specifies 32 bit signed integers unless stated otherwise.
...
enum_name_replacements = {
    # Get/Set common suffix merging produces names starting with "et".
    # Use the names from the 2.1 spec instead.
    "etVariableDataEntryEntriesAttributeType": "AttributeEnumType",
    "etVariableResultAttributeType": "GetVariableResultAttributeEnumType",
}

# Messages for provisioning against a CSMS.
# Grown message by message.
provisioning_profile = [[ #send
        schema21.BootNotificationRequest.BootNotificationRequest,
        schema21.HeartbeatRequest.HeartbeatRequest,
        schema21.StatusNotificationRequest.StatusNotificationRequest,
        schema21.GetVariablesResponse.GetVariablesResponse,
        schema21.SetVariablesResponse.SetVariablesResponse,
        schema21.GetBaseReportResponse.GetBaseReportResponse,
        schema21.TriggerMessageResponse.TriggerMessageResponse,
        schema21.ResetResponse.ResetResponse,
    ], [ #recv
        schema21.BootNotificationResponse.BootNotificationResponse,
        schema21.HeartbeatResponse.HeartbeatResponse,
        schema21.StatusNotificationResponse.StatusNotificationResponse,
        schema21.GetVariablesRequest.GetVariablesRequest,
        schema21.SetVariablesRequest.SetVariablesRequest,
        schema21.GetBaseReportRequest.GetBaseReportRequest,
        schema21.TriggerMessageRequest.TriggerMessageRequest,
...
def all_action_names():
    names = []
    for f in sorted(os.listdir("../spec/schemas-2.1/json")):
        if not f.endswith(".json"):
            continue
        names.append(f.removesuffix(".json").removesuffix("Request"))
    return names
...
def props(obj):
    return {k: v for k, v in obj.properties.items() if k != "customData"}
...
def param_size(param_name, e: Element):
    if isinstance(e, String):
        ...
        #return flatten([param_size("lalala" + propname, prop.element) for propname, prop in e.properties.items()]) + ["JSON_OBJECT_SIZE({})".format(len(props(e)))]
...
"""
    ...
    h_content = template.format(name=name,
        members=wrap_non_empty("", ";\n    ".join([param_arg(name, k, v, strings_as_arrays=False).replace(",", ";") for k, v in props(obj).items()]), ";"))
    cpp_content = impl_template.format(name=name,
                                       serialization="\n        ".join([param_insertion(name, k, v) for k, v in props(obj).items()]),
                                       member_count=len(props(obj).items()),
                                       param_sizes="\n".join([param_size(k, v.element) for k, v in props(obj).items()]))
...
"""
    ...
    for param_name, param in props(obj).items():
...
def generate_recv_function(obj: Object, nameoverride=None, top_level=False):
    ...
    if (obj.size() != keys_handled) {{
        return CallResponse{{CallErrorCode::FormatViolation, "{name}: unknown members passed"}};
    }}
    ...
    if (!obj.containsKey("{member}"))
        return CallResponse{{CallErrorCode::OccurrenceConstraintViolation, "{member}: required, but missing"}};
"""
    ...
    # customData is allowed on every 2.1 type. We ignore its content
    # but must count the key to not trip the unknown member check.
    parse_members.append("""if (obj.containsKey("customData")) ++keys_handled;""")
...
"""
    ...
    default_cases=["case CallAction::{}:".format(camel_to_upper_snake(x)) for x in all_action_name_list]
...
if __name__ == "__main__":
    ...
    h_callactionenum, cpp_callactionenum = generate_enum("CallAction", all_action_names(), add_none=False)
    enum_decls.append(h_callactionenum)
    enum_impls.append(cpp_callactionenum)

    h, cpp = generate_on_call(all_action_names(), flatten([x[1] for x in supported_profiles]))
    h2, cpp2 = generate_on_call_response(all_action_names(), flatten([x[1] for x in supported_profiles]))
    ...
    specialize_template("Messages21.h.template", "generated/Messages21.h", {
    ...
    specialize_template("Messages21.cpp.template", "generated/Messages21.cpp", {
