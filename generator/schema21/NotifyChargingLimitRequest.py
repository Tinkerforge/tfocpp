...
class ChargingLimitType(Object, additionalProperties=False):

    chargingLimitSource: str = Property(String(maxLength=20, description='Represents the source of the charging limit. Values defined in appendix as ChargingLimitSourceEnumStringType.\r\n'), required=True)

    isLocalGeneration: Maybe[bool] = Property(Boolean(description='*(2.1)* True when the reported limit concerns local generation that is providing extra capacity, instead of a limitation.\r\n'))

    isGridCritical: Maybe[bool] = Property(Boolean(description='Indicates whether the charging limit is critical for the grid.\r\n'))
...
class NotifyChargingLimitRequest(Object, additionalProperties=False):

    chargingSchedule: Maybe[List[ChargingScheduleType]] = Property(Array(ChargingScheduleType, additionalItems=False, minItems=1))

    evseId: Maybe[int] = Property(Integer(minimum=0.0, description='The EVSE to which the charging limit is set. If absent or when zero, it applies to the entire Charging Station.\r\n'))

    chargingLimit: ChargingLimitType = Property(ChargingLimitType, required=True)
