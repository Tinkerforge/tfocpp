...
class GetLogResponse(Object, additionalProperties=False):

    status: str = Property(String(enum=['Accepted', 'Rejected', 'AcceptedCanceled'], description='This field indicates whether the Charging Station was able to accept the request.\r\n'), required=True)

    statusInfo: Maybe[StatusInfoType] = Property(StatusInfoType)

    filename: Maybe[str] = Property(String(maxLength=255, description='This contains the name of the log file that will be uploaded. This field is not present when no logging information is available.\r\n'))
