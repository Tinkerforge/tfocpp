...
class ReportChargingProfilesRequest(Object, additionalProperties=False):

    requestId: int = Property(Integer(description='Id used to match the &lt;&lt;getchargingprofilesrequest, GetChargingProfilesRequest&gt;&gt; message with the resulting ReportChargingProfilesRequest messages. When the CSMS provided a requestId in the &lt;&lt;getchargingprofilesrequest, GetChargingProfilesRequest&gt;&gt;, this field SHALL contain the same value.\r\n'), required=True)

    chargingLimitSource: str = Property(String(maxLength=20, description='Source that has installed this charging profile. Values defined in Appendix as ChargingLimitSourceEnumStringType.\r\n'), required=True)

    chargingProfile: List[ChargingProfileType] = Property(Array(ChargingProfileType, additionalItems=False, minItems=1), required=True)

    tbc: bool = Property(Boolean(default=False, description='To Be Continued. Default value when omitted: false. false indicates that there are no further messages as part of this report.\r\n'))

    evseId: int = Property(Integer(minimum=0.0, description='The evse to which the charging profile applies. If evseId = 0, the message contains an overall limit for the Charging Station.\r\n'), required=True)
