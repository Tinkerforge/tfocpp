...
class FixedVarType(Object, additionalProperties=False):
    ...
    unit: str = Property(String(enum=['Not_Applicable', 'PctMaxW', 'PctMaxVar', 'PctWAvail', 'PctVarAvail', 'PctEffectiveV'], description='Unit of the setpoint.\r\n'), required=True)
...
class DERCurveType(Object, additionalProperties=False):
    ...
    yUnit: str = Property(String(enum=['Not_Applicable', 'PctMaxW', 'PctMaxVar', 'PctWAvail', 'PctVarAvail', 'PctEffectiveV'], description='Unit of the setpoint.\r\n'), required=True)
...
class SetDERControlRequest(Object, additionalProperties=False):

    isDefault: bool = Property(Boolean(description='True if this is a default DER control\r\n\r\n'), required=True)

    controlId: str = Property(String(maxLength=36, description='Unique id of this control, e.g. UUID\r\n\r\n'), required=True)

    controlType: str = Property(String(enum=['EnterService', 'FreqDroop', 'FreqWatt', 'FixedPFAbsorb', 'FixedPFInject', 'FixedVar', 'Gradients', 'HFMustTrip', 'HFMayTrip', 'HVMustTrip', 'HVMomCess', 'HVMayTrip', 'LimitMaxDischarge', 'LFMustTrip', 'LVMustTrip', 'LVMomCess', 'LVMayTrip', 'PowerMonitoringMustTrip', 'VoltVar', 'VoltWatt', 'WattPF', 'WattVar'], description='Type of control.  Determines which setting field below is used.\r\n\r\n'), required=True)

    curve: Maybe[DERCurveType] = Property(DERCurveType)

    enterService: Maybe[EnterServiceType] = Property(EnterServiceType)

    fixedPFAbsorb: Maybe[FixedPFType] = Property(FixedPFType)

    fixedPFInject: Maybe[FixedPFType] = Property(FixedPFType)

    fixedVar: Maybe[FixedVarType] = Property(FixedVarType)

    freqDroop: Maybe[FreqDroopType] = Property(FreqDroopType)

    gradient: Maybe[GradientType] = Property(GradientType)

    limitMaxDischarge: Maybe[LimitMaxDischargeType] = Property(LimitMaxDischargeType)
