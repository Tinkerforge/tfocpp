...
class RequestStartTransactionRequest(Object, additionalProperties=False):

    evseId: Maybe[int] = Property(Integer(minimum=1.0, description='Number of the EVSE on which to start the transaction. EvseId SHALL be &gt; 0\r\n'))

    groupIdToken: Maybe[IdTokenType] = Property(IdTokenType)

    idToken: IdTokenType = Property(IdTokenType, required=True)

    remoteStartId: int = Property(Integer(description='Id given by the server to this start request. The Charging Station will return this in the &lt;&lt;transactioneventrequest, TransactionEventRequest&gt;&gt;, letting the server know which transaction was started for this request. Use to start a transaction.\r\n'), required=True)

    chargingProfile: Maybe[ChargingProfileType] = Property(ChargingProfileType)
