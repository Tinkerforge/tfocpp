...
class NotifyEVChargingScheduleRequest(Object, additionalProperties=False):

    timeBase: str = Property(String(format='date-time', description='Periods contained in the charging profile are relative to this point in time.\r\n'), required=True)

    chargingSchedule: ChargingScheduleType = Property(ChargingScheduleType, required=True)

    evseId: int = Property(Integer(minimum=1.0, description='The charging schedule contained in this notification applies to an EVSE. EvseId must be &gt; 0.\r\n'), required=True)

    selectedChargingScheduleId: Maybe[int] = Property(Integer(minimum=0.0, description='*(2.1)* Id  of the _chargingSchedule_ that EV selected from the provided ChargingProfile.\r\n'))

    powerToleranceAcceptance: Maybe[bool] = Property(Boolean(description='*(2.1)* True when power tolerance is accepted by EV.\r\nThis value is taken from EVPowerProfile.PowerToleranceAcceptance in the ISO 15118-20 PowerDeliverReq message..\r\n'))
