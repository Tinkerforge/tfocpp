...
class CertificateStatusType(Object, additionalProperties=False):
...
"""
    ...
    status: str = Property(String(enum=['Good', 'Revoked', 'Unknown', 'Failed'], description='Status of certificate: good, revoked or unknown.\r\n'), required=True)

    nextUpdate: str = Property(String(format='date-time'), required=True)

    customData: Maybe[CustomDataType] = Property(CustomDataType)


class GetCertificateChainStatusResponse(Object, additionalProperties=False):

    certificateStatus: List[CertificateStatusType] = Property(Array(CertificateStatusType, additionalItems=False, minItems=1, maxItems=4), required=True)
