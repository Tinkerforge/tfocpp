...
class AuthorizeRequest(Object, additionalProperties=False):
    ...
    certificate: Maybe[str] = Property(String(maxLength=10000, description='*(2.1)* The X.509 certificate chain presented by EV and encoded in PEM format. Order of certificates in chain is from leaf up to (but excluding) root certificate. +\r\nOnly needed in case of central contract validation when Charging Station cannot validate the contract certificate.\r\n\r\n'))

    iso15118CertificateHashData: Maybe[List[OCSPRequestDataType]] = Property(Array(OCSPRequestDataType, additionalItems=False, minItems=1, maxItems=4))
