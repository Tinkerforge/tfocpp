...
public:
    ...
    QueueItem() : action(CallAction::BOOT_NOTIFICATION), buf(nullptr), message_id(0), len(0) {}

    QueueItem(const ICall &call);
...
class Connection {
public:
    void *start(const char *websocket_endpoint_url, const char *charge_point_name_percent_encoded, const char *basic_auth_pass, const PlatformTlsConfig *tls, ChargePoint *ocpp_handle);
